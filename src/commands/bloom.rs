...
pub fn bloom_filter_add2_value(ctx: &Context, input_args: &Vec<RedisString>) -> RedisResult {
    let argc = input_args.len();
    if argc != 3 {
        return Err(RedisError::Str(ERROR));
    }
...
pub fn bloom_filter_exists2(ctx: &Context, input_args: &Vec<RedisString>) -> RedisResult {
    let argc = input_args.len();
    if argc != 3 {
        return Err(RedisError::Str(ERROR));
    }
